...
void MainWindow::timerEvent( QTimerEvent * )
{
    if ( !initializing )
    {
        ...
        us_rawnet_multi_rawnet_poll_incoming( &multi_rawnet, cur_time, 1024, this, incomingAvdeccPacketCallback );
    }
}
    ...
    if ( !initializing )
    {
        ...
        if ( ui->checkBoxEntityID->isChecked() )
        {
            ...
            if ( ok )
            {
                ...
                ui->lineEditEntityID->setText(
                    QString::fromLatin1( "0x%1" ).arg( option_entity, 16, 16, QChar::fromLatin1( '0' ) ) );
            }
        }
        else
        {
...
void MainWindow::incomingAvdeccPacket( us_rawnet_multi_t *multiport, int ethernet_port, uint8_t *buf, uint16_t len )
{
    (void)multiport;
    (void)ethernet_port;
    char text[4096] = "";
...
void MainWindow::incomingAvdeccPacketCallback(
    us_rawnet_multi_t *self, int ethernet_port, void *context, uint8_t *buf, uint16_t len )
{
    MainWindow *win = (MainWindow *)context;
    if ( win->global_enable )
    {
        win->incomingAvdeccPacket( self, ethernet_port, buf, len );
    }
}
